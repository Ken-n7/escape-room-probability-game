...
-- ── perf_samples: ANONYMOUS client performance telemetry ─────────────────────
-- One aggregated row per player per minute of gameplay. Deliberately has NO
-- user link — it exists only to learn which devices/GPUs struggle so graphics
-- can be tuned. session_id is a random per-tab UUID, never tied to auth.
create table if not exists public.perf_samples (
  id            bigint generated always as identity primary key,
  session_id    uuid not null,                 -- random per-tab, anonymous
  minute_index  int  not null default 0,       -- which minute of the session (0,1,2…)
  fps_avg       real not null,
  fps_min       real,
  fps_p05       real,                           -- 5th percentile = worst sustained fps
  frames        int,                            -- frames aggregated into this row
  dpr           real,                           -- devicePixelRatio
  screen_w      int,
  screen_h      int,
  device_memory real,                           -- navigator.deviceMemory (GB, coarse)
  cpu_cores     int,                            -- hardwareConcurrency
  is_mobile     boolean,
  platform      text,
  gpu           text,                           -- WebGL UNMASKED_RENDERER (real chip)
  ua            text,                           -- user-agent (coarse device/OS)
  app_version   text,                           -- build tag, if provided
  created_at    timestamptz not null default now()
);
create index if not exists perf_session_idx on public.perf_samples (session_id);
create index if not exists perf_created_idx on public.perf_samples (created_at);
create index if not exists perf_gpu_idx     on public.perf_samples (gpu);

alter table public.perf_samples enable row level security;

-- Anyone (including anonymous, signed-out players) may INSERT; only admins may
-- read. No update/delete policy — rows are write-once. The insert is unauthenticated
-- by design so perf data is captured even before/without login.
drop policy if exists "perf_insert" on public.perf_samples;
create policy "perf_insert" on public.perf_samples for insert with check ( true );
drop policy if exists "perf_select" on public.perf_samples;
create policy "perf_select" on public.perf_samples for select using ( public.is_admin() );

grant insert on public.perf_samples to anon, authenticated;
grant select on public.perf_samples to authenticated;
